...
项目不以功能数量为目标，不通过新增无关服务、无关页面或无关业务模块扩大规模；前端完整演示系统属于已明确的必做交付范围，不视为无关扩展。
...
- Java 21 全模块构建通过；
- Seata Server 2.0.0 启动、注册和 DB Store 已验证；
- Nacos YAML 语法已修复；
- MySQL、Redis、Nacos 基础运行验证通过。

### 当前阶段

基础设施运行基线已完成，核心业务链路已验收通过。Seata Server 基线完成不等于 Seata AT 业务回滚已完成。

已实现并验证：

- Java 21 / Maven / MySQL / Redis / Nacos / Seata Server 基线；
- User/Auth/Gateway（登录、Token 类型隔离、黑名单、用户上下文透传）；
- mall-user 停止/恢复故障场景；
- Cart（无 Token 拒绝、有 Token 访问、加入购物车）；
- Order 主链路（商品查询、库存锁定、订单创建）；
- Seata AT 回滚（故障注入、订单未落库、库存恢复）；
