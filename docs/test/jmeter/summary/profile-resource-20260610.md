...
| full-docker-profile | `cmd /c start-all.bat --profile full --no-build --skip-frontend --allow-partial --no-pause` | 通过 | RocketMQ Console 镜像已替换为 `apacherocketmq/rocketmq-dashboard:latest`，全量 Docker 启动通过 | 已进入后端启动 | 状态文件已生成 | 未执行 |
...
| full-docker-profile | 未采样 | 未采样 | 未采样 | RocketMQ Console 镜像替换后，全量 Docker 启动通过 |
