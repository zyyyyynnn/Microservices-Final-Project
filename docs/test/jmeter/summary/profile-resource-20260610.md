...
| full-docker-profile | `cmd /c start-all.bat --profile full --no-build --skip-frontend --allow-partial --no-pause` | 未通过 | Docker 拉取 `rocketmq-console-ng`、`zipkin`、`kibana` 时镜像源 403 | 未进入后端启动 | 未生成后端状态文件 | 未执行 |
...
| full-docker-profile | 未采样 | 未采样 | 未采样 | Docker 镜像源 403，未启动 |
