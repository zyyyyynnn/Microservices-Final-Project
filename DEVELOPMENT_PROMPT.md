...
- JMeter 脚本已建立，搜索负载、订单短冒烟、秒杀 1 用户完整链路、秒杀 10 用户完整链路连续 3 次和秒杀 50/100/200/300/500 固定库存阶梯压力已执行；订单正式负载仍待执行。
- Elasticsearch 搜索、Sentinel 规则限流/熔断、Nacos 普通业务配置热更新已完成专项验收。
- 秒杀 10 用户完整链路阻断已修复并完成连续 3 次验证；100/500 用户结果轮询窗口不足问题已通过 `ResultPollAttempts=120`、`ResultPollDelayMs=500` 复测收口。
