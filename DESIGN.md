...
- 解决界面“空、假”感：所有商品图必须使用真实摄影图（Unsplash 等公开免费源），不再使用色块+文字标签式 SVG 占位图；首页 hero 区去除装饰性渐变海报，改为品牌头 + 交易链路线条的极简结构；SKUSelector、ShipForm、AdminTable 抽为独立组件以提升复用与一致性。
...
### 6.1 前台商城页面
- **首页 (HomeView)**:
  - 顶部为紧凑的品牌头（品牌名 + 一级导航 + 搜索入口），不再使用大尺寸渐变 hero-banner 海报；
  - 品牌头下方为「交易链路线条」：以横向时间线/步骤条形式展示「登录 → 浏览 → 加购 → 下单 → 支付 → 完成」节点，节点支持点击跳转对应路由，替代原 hero-banner + flow-panel 双栏布局；
  - 核心频道（如数码家电、美妆护肤等）以品类卡片网格形式呈现，支持点击一键切换搜索检索；
  - 推荐商品列表及限时秒杀展示区采用规范网格排列，不可被拉伸或挤压；
  - 商品图必须使用真实摄影图（Unsplash 等公开源），禁止色块+文字标签式 SVG 占位图。
- **商品详情 (ProductDetailView)**:
...
- **ProductImage (商品图)**:
  - 商品主图必须使用真实摄影图 URL（Unsplash 等公开免费可商用源），通过 `src/catalog/productAssets.ts` 中的 `productImagesBySpuId` 映射表统一管理。
  - 严禁使用 AI 生成图、严禁使用色块+文字标签式 SVG 占位图作为正式商品主图。原 13 张占位 SVG 仅作为图片加载失败的 fallback。
  - 商品图缺失时使用统一占位图，且占位图必须明确表达“图片缺失 / 暂无图片”。ProductImage 组件必须支持 fallback。
  - 图片加载采用 lazy loading，`<img loading="lazy" />`，避免首屏阻塞；首屏商品图可加 `fetchpriority="high"`。

- **SKUSelector (SKU 选择器)** — 已抽为独立组件 `src/components/SKUSelector.vue`:
  ...
  - Props: `options: SkuOption[]`、`modelValue: string | null`、`disabled: boolean`；Emits: `update:modelValue`。
- **Price (价格组件)**:
...
- **AdminTable (后台表格)** — 已抽为独立组件 `src/components/AdminTable.vue`:
  - 追求扫描效率和紧凑的信息密度。商品列单列整合缩略图、商品名称与 SPU 编号。字段缺失均展示为 `—`。
  - Props: `columns: TableColumn[]`、`data: T[]`、`loading: boolean`、`emptyText: string`；插槽支持自定义单元格。
- **ShipForm (发货单)** — 已抽为独立组件 `src/components/ShipForm.vue`:
  - 采用上下分层的极简 `ship-query` 并行布局结构，去除 placeholder，输入框与发货按钮横向对齐并支持窄屏折行。
  - Props: `orderId: string`、`loading: boolean`；Emits: `submit(orderId: string)`。
...
5. **商品展示主图必须使用真实摄影图 URL（Unsplash 等公开免费可商用源），不得使用 AI 生成图、不得使用色块+文字标签式 SVG 占位图作为正式商品主图**。原 SVG 占位图仅作为加载失败兜底。

---

### 10.14 Image Asset Policy
- **商品主图源**: 统一使用 Unsplash 公开免费可商用摄影图（`https://images.unsplash.com/photo-...?w=600&h=600&fit=crop`），通过 `src/catalog/productAssets.ts` 的 `productImagesBySpuId` 映射表管理。
- **品类导航图**: 同样使用 Unsplash 真实摄影图，按品类语义选图（手机数码/美妆护肤/食品生鲜等）。
- **占位图**: 加载失败时使用 `src/assets/placeholder.svg`，明确表达「图片缺失」语义。
- **禁用资源**:
  - 禁用原 `public/products/*.svg` 中 13 张色块+文字标签式占位图作为正式商品主图；
  - 禁用 `src/assets/hero/mallcloud-hero.png` 死代码资产；
  - 禁用 `app.css` 中 `.commerce-hero` / `.hero-visual` 整套遗留样式块。
- **图片尺寸规范**: 商品主图统一 600×600（Unsplash `w=600&h=600&fit=crop`）；品类导航图统一 200×200；列表缩略图统一 120×120。
- **性能**: 所有 `<img>` 必须 `loading="lazy"`；首屏商品图可加 `fetchpriority="high"`。
...
| 2026-07-01 | **UI 精致感升级** | 删除首页 hero-banner 渐变海报，改为品牌头+交易链路线条布局；商品图从 SVG 占位升级为 Unsplash 真实摄影图；SKUSelector/ShipForm/AdminTable 抽为独立组件；删除移动端截图与 sprint3 过程证据；补 shadow-lg/xl、transition-slow token；明确商品图真实摄影图政策（10.14 节）。 |
