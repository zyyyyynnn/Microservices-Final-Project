...
- 保持文档与代码同步，所有未通过最终网页端人工复验和视觉验收的页面与状态一律标记为“待网页端复审，待视觉验收”或“视觉不合格，待重构”。
...
|---|---|---|---|---|
| 游客 | `docs/PRD.md`、Gateway 白名单 | 浏览公共商品信息、分类展示与检索搜索 | 公共商品详情、分类树、搜索结果、热门搜索词、账户注册与登录 | 能力已定义，待网页端复审 |
| 普通用户（`USER`） | `docs/QUICK_START.md` | 体验完整的购物流程及核心订单交易链路 | 个人信息修改、地址簿管理、购物车管理、订单创建、模拟支付、秒杀参与 | 能力已定义，待网页端复审 |
| 商家（`MERCHANT`） | `docs/PRD.md`、`docs/QUICK_START.md` | 课程演示所需的商品和订单基础维护 | 后台订单列表、发货、商品列表浏览及商品上下架 | 权限边界待确认，后台登录待确认 |
| 管理员（`ADMIN`） | `docs/PRD.md` | 后台全部指标浏览与核心后台演示操作 | 后台看板、订单及商品全局管理；独立管理员登录流程 | 权限边界待确认，后台登录待确认 |
...
| 首页 | `/` | 游客/用户 | `GET /api/v1/categories/tree` | 已存在，视觉不合格，待重构 |
| 商品详情页 | `/products/:id` | 游客/用户 | `GET /api/v1/products/{id}` | 已存在，视觉不合格，待重构 |
| 商品搜索页 | `/search` | 游客/用户 | `GET /api/v1/search/products` | 已存在，视觉不合格，待重构 |
| 购物车页 | `/cart` | 用户 | `GET /api/v1/carts` | 已存在，视觉不合格，待重构 |
| 个人账户页 | `/account` | 用户 | `GET /api/v1/users/me`、地址 CRUD | 已存在，视觉不合格，待重构 |
| 确认结算页 | `/checkout` | 用户 | `POST /api/v1/orders` | 已存在，视觉不合格，待重构 |
| 订单详情页 | `/orders/:orderNo` | 用户 | `GET /api/v1/orders/{orderNo}` | 已存在, 视觉不合格，待重构 |
| 模拟支付页 | `/pay/:orderNo` | 用户 | `POST /api/v1/pay/create` | 已存在，视觉不合格，待重构 |
| 秒杀活动页 | `/seckill` | 用户 | 秒杀列表、下单及结果轮询接口 | 已存在，视觉不合格，待重构 |
...
设计系统 v2.0 的核心设计风格为：**低饱和雾霾蓝极简 + 轻电商质感**。首页采用首屏高级极简与商品区轻电商的混合设计方向，前后台共享基础 token，但允许前台与后台采用不同的信息密度。
...
### 10.6 Shadow
在 v2.0 中，允许引入极其克制的浅阴影来建立视觉层级关系。
...
### 13.1 一致性与落地审计
- 所有的页面视觉设计及布局必须与 `DESIGN.md v2.0` 的标准一致，所有路由对应的页面已存在。
...
### 13.3 规范与兼容审计
- `npm run build` 命令必须无警告、无错误地编译完成；`git diff --check` 执行结果返回为 0。
...
| 页面名称 | 路由路径 | 必须具备 | 当前状态 |
|---|---|---|---|
| 首页 / 商品推荐 | `/` | 类目入口、商品卡片、商品详情入口、搜索入口 | 已存在，视觉不合格，待重构 |
| 商品详情 | `/products/:id` | 商品标题、价格、SKU、库存状态、数量、加入购物车、错误状态 | 已存在，视觉不合格，待重构 |
| 搜索页 | `/search` | 关键字、热词、结果列表、分页、空结果、错误状态 | 已存在，视觉不合格，待重构 |
| 登录页 | `/login` | 登录表单、错误提示、跳转来源 | 已存在，待网页端复审，待视觉验收 |
| 注册页 | `/register` | 独立注册表单、字段校验、成功后登录引导 | 已存在，待网页端复审，待视觉验收 |
| 个人账户页 | `/account` | 用户资料、地址列表、新增入口、空状态 | 已存在，视觉不合格，待重构 |
| 购物车页 | `/cart` | 商品行、价格、小计、勾选、数量、删除、结算、空状态 | 已存在，视觉不合格，待重构 |
| 确认结算页 | `/checkout` | 地址选择、订单项确认、金额汇总、提交状态 | 已存在，视觉不合格，待重构 |
| 订单详情页 | `/orders/:orderNo` | 订单状态、商品明细、金额、支付入口、错误状态 | 已存在，视觉不合格，待重构 |
| 模拟支付页 | `/pay/:orderNo` | 支付记录、模拟支付说明、通知结果、查询订单入口 | 已存在，视觉不合格，待重构 |
| 秒杀活动页 | `/seckill` | 活动列表、活动详情、状态文案、发起请求、结果轮询 | 已存在，视觉不合格，待重构 |
| 后台管理页 | `/admin` | 看板统计、订单表格、商品表格、发货操作 | 已存在，待网页端复审，待视觉验收 |
...
| 2026-06-11 | 后台管理页 规范收口 | 清理所有带有 `AdminController` 开发联调色彩的警示条，商品列重构为图文卡片。 |
| 2026-06-11 | **确立设计基线 v2.0** | 确立**低饱和雾霾蓝极简 + 轻电商质感**；Hero 背景图解锁允许重构，阴影和极简过渡有条件解禁；对金额和状态做强兜底以显示为 `—`。 |
