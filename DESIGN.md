...
> 前端技术栈：Vue 3 + Vite + TypeScript + Element Plus + Axios + Pinia（来自 `README.md`，当前仓库未检出前端源码）
...
当前仓库未检出 Vue 前端目录、路由文件、页面文件或样式 token 文件。以下信息架构是基于已确认后端接口和项目文档建立的产品基线，前端实际路由文件待确认。前端完整演示系统为必做阶段，DESIGN.md 是该阶段实现依据。
...
前端路由文件当前未检出，以下「前端路由」是设计基线建议路径，需在前端实现时确认；「Gateway / API」列只使用仓库已确认的路径。
...
以下视觉规则来自原 `DESIGN.md`，保留为 MallCloud 前端实现的设计系统基线。当前仓库未检出 `web-portal` / `web-admin`，因此具体文件路径为待确认；一旦创建前端工程，必须复用本节 token 和 Element Plus 映射，不另起一套组件库或设计系统。
...
当前仓库未检出前端源码，以下文件路径为待确认。前端工程创建或接入时应使用同一顺序导入：
...
| 步骤 | 动作 | 文件 |
|---|---|---|
| 1 | 引入全局重置和禁动效 | `src/styles/reset.css`（待确认） |
| 2 | 引入 token 变量 | `src/styles/tokens.css`（待确认） |
| 3 | 覆盖 Element Plus 主题 | `src/styles/element-theme.css`（待确认） |
| 4 | 在前端入口导入全局样式 | `src/main.ts`（待确认） |
...
- 待确认：当前仓库未检出 Vue 前端工程，`web-portal`、`web-admin`、`src/main.ts`、路由、页面、组件和样式 token 文件是否在其他仓库或尚未创建。
