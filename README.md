...
| JMeter | 搜索负载场景已执行且零失败，订单短冒烟和秒杀固定库存阶梯压力已执行且零失败；订单正式负载待执行 | 搜索 1/50/150 用户、订单 1 用户短冒烟、秒杀 1 用户完整链路、10 用户完整链路连续 3 次、50/100/200/300/500 固定库存阶梯均已完成；秒杀阶梯指标为 JMeter Total 样本聚合，不等同于最终订单 TPS |
...
开发环境通过 Vite 代理把 `/api/v1/**` 转发到 `http://localhost:9100`。前端不得直接调用内部微服务端口，不得使用 mock 伪造后端未完成能力。
