...
| JMeter | 搜索负载场景已执行且零失败，订单短冒烟和秒杀 10 用户完整链路已执行且零失败；秒杀阶梯压力已阶段执行但未验收通过 | 搜索 1/50/150 用户、订单 1 用户短冒烟、秒杀 10 用户请求受理短冒烟、1 用户完整链路短冒烟和 10 用户完整链路连续 3 次验证已完成；秒杀 50/100/200/300/500 阶梯已有阶段结果，其中 100/500 用户存在失败样本；订单正式负载待执行 |
...
- Docker 全栈不是当前正式交付路径；
- Kubernetes 仅保留部分示例；
- JMeter 订单正式负载待执行；秒杀 50/100/200/300/500 阶梯压力已有阶段结果，但 100/500 用户存在失败样本，尚未验收通过；
