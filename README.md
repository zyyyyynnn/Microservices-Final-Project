...
| JMeter | 搜索负载场景已执行且零失败，订单短冒烟和秒杀 1 用户完整链路短冒烟已执行且零失败 | 搜索 1/50/150 用户、订单 1 用户短冒烟、秒杀 10 用户请求受理短冒烟和 1 用户完整链路短冒烟已完成；秒杀 10 用户完整链路验证已记录为阻断，订单正式负载和秒杀阶梯压力待执行 |
...
- Docker 全栈不是当前正式交付路径；
- Kubernetes 仅保留部分示例；
- JMeter 订单正式负载、秒杀阶梯压力测试待执行；秒杀 10 用户完整链路仍有异步最终状态阻断；
